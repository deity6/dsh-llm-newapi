...
- `0.9.13`：**排版大重构（分段控件 + 卡组导航 + 下一级编辑页）**。
  - 顶部 tab 改为**滑动分段控件**（图1 风格）：[配置] [设置] [回收站]，回收站
    段带计数 badge；同一行右侧保留保存状态条 + 手动保存按钮（防忘保存）。
  - 配置页改为**卡组列表**（图3 风格）：每实例一行圆形协议图标 + 名称 +
    「协议：xx · 已配置 N 个模型」副标题 + 右侧箭头；点击进入**实例编辑页**
    （下一级，带返回按钮）。
  - 设置页改为**分区卡组**：通用（删除找回提示/删除音效/删除撤销/撤销时长）
    + 数据管理（从描述符导入）；每行「标题 + 描述 + 右侧控件」统一排版。
  - 回收站改为**独立第三页**（原内嵌弹窗废弃）：列表 + 恢复/彻底删除/清空；
    彻底删除/清空仍弹大窗口确认（不可逆）。
  - **删除找回提示**：实例删除（移入回收站）后首次弹 toast「可在回收站找回」，
    之后静默；设置页「删除找回提示」开关可重新开启（`ui.deleteRecoverHint`，
    默认 true，持久化）。
  - **删除按钮弱化**：实例卡/编辑页头部垃圾桶图标隐藏（hideRemoveButton），
