...
import { assertUsableApiKey as assertUsableApiKey2, LlmError as LlmError6, resolveRetryPolicy, RetryPolicySchema } from "@deepseek-ai/dsh-llm";
...
var NewApiAdapter = class extends LlmAdapter {
  ...
  async fetchModelList(base, apiKey, connection, signal) {
    ...
    try {
      ...
      throw new LlmError5(`NewAPI model discovery request to ${base} failed`, "TRANSPORT", { cause: error });
    }
    if (!response.ok) {
      let providerError;
      try {
    ...
    try {
      ...
      throw new LlmError5(`NewAPI model discovery from ${base} returned a malformed body`, "MALFORMED_RESPONSE");
    }
  }
  /**
   * Interrogate one gateway endpoint for the models it advertises, serving
   ...
   */
  async probeChat(base, apiKey, proxyUrl, headers, request) {
    ...
    try {
      ...
      return {
        ...
        status: response.status,
        latencyMs,
        ...choice?.message?.content !== void 0 ? { text: choice.message.content } : {},
        ...choice?.finish_reason !== void 0 ? { finishReason: choice.finish_reason } : {}
      };
    } catch (error) {
      const latencyMs = Date.now() - chatStarted;
      if (request.signal?.aborted) {
   ...
   */
  async probeToolCall(base, apiKey, proxyUrl, headers, request) {
    const started = Date.now();
  ...
  async fetchModelsDevParams(request, signal) {
    ...
    try {
      ...
      if (error instanceof LlmError5) throw error;
      if (signal.aborted) throw error;
    ...
    try {
      ...
      try {
        ...
        if (options.signal?.aborted) {
          throw new LlmError5("NewAPI request aborted by caller", "ABORTED", { cause: error });
        }
        if (error instanceof LlmError5) throw error;
        throw new LlmError5(`NewAPI stream from ${connection.baseURL} failed`, "TRANSPORT", { cause: error });
      } finally {
  ...
  async *request(options, signal, connection, apiKey, onComment) {
    ...
    if (!response.body) {
      throw new LlmError5("NewAPI returned no response body", "EMPTY_RESPONSE");
    }
  ...
  async raiseForResponse(response, base) {
    ...
    const id = requestId(response.headers);
    throw new LlmError5(message, httpErrorCode(response.status, providerError), {
      status: response.status,
