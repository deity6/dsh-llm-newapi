...
import { assertUsableApiKey as assertUsableApiKey2, LlmError as LlmError7, resolveRetryPolicy, RetryPolicySchema } from "@deepseek-ai/dsh-llm";
...
var DONE = "[DONE]";
async function* parseSse(stream, onComment, expectDone = true) {
...
// src/efforts.ts
var OPENAI_FULL = { efforts: ["none", "low", "medium", "high"], defaultEffort: "medium" };
var DEEPSEEK_HYBRID = { efforts: ["none", "low", "medium", "high"], defaultEffort: "none" };
var DEEPSEEK_REASONER = { efforts: ["low", "medium", "high"], defaultEffort: "medium" };
var THREE_RUNG = { efforts: ["low", "medium", "high"], defaultEffort: "medium" };
function defaultReasoningEffortsFor(modelId) {
...
function flattenText2(blocks) {
  return blocks.filter((block) => block.type === "text").map((block) => block.text).join("");
}
function parseInput(argumentsText) {
...
function anthropicAssistant(blocks) {
  const content = [];
  const text = flattenText2(blocks);
...
function serializeAnthropicMessages(messages, system) {
  const systemParts = [];
  const wire = [];
  for (const message of messages) {
    if (message.role === "system") {
      const text2 = flattenText2(message.content);
      if (text2.length > 0) systemParts.push(text2);
      continue;
    }
    if (message.role === "assistant") {
    ...
    const content = [];
    const text = flattenText2(message.content);
    if (text.length > 0) content.push({ type: "text", text });
    for (const block of message.content) {
      ...
      content.push({
        ...
        content: flattenText2(block.content) || "(no output)"
      });
    }
    if (content.length > 0) wire.push({ role: "user", content });
  }
  if (system !== void 0 && system.length > 0) systemParts.unshift(system);
  return {
...
async function* anthropicEventsToWire(payloads) {
  const toolIndices = /* @__PURE__ */ new Map();
  let nextToolIndex = 0;
  let pendingUsage;
  let sawMessageStart = false;
  for await (const payload of payloads) {
    if (payload === DONE) {
      throw new LlmError4("Anthropic SSE stream ended without message_stop", "STREAM_CLOSED");
    }
    let event;
    try {
      event = JSON.parse(payload);
    } catch {
      throw new LlmError4(`malformed Anthropic SSE payload: ${payload.slice(0, 120)}`, "MALFORMED_RESPONSE");
    }
    switch (event.type) {
      case "message_start": {
        sawMessageStart = true;
        pendingUsage = event.message?.usage === void 0 ? pendingUsage : mapAnthropicUsage(event.message.usage);
        continue;
      }
      case "content_block_start": {
        const block = event.content_block;
        if (block?.type === "tool_use" && block.id !== void 0 && block.name !== void 0) {
          const wireIndex = nextToolIndex++;
          if (event.index !== void 0) toolIndices.set(event.index, wireIndex);
      ...
      case "content_block_delta": {
        ...
        if (type === "text_delta" && typeof delta.text === "string" && delta.text.length > 0) {
          ...
          const wireIndex = event.index !== void 0 ? toolIndices.get(event.index) : void 0;
          yield JSON.stringify({
            choices: [{
              index: 0,
              delta: {
      ...
      case "message_delta": {
        const reason = mapStopReason(event.delta?.stop_reason ?? void 0);
        if (event.usage !== void 0) pendingUsage = mapAnthropicUsage(event.usage);
        if (reason !== void 0) {
...
function flattenText3(blocks) {
  return blocks.filter((block) => block.type === "text").map((block) => block.text).join("");
}
function serializeResponsesInput(messages, system) {
  const instructions = [];
  if (system !== void 0 && system.length > 0) instructions.push(system);
  const input = [];
  for (const message of messages) {
    if (message.role === "system") {
      const text2 = flattenText3(message.content);
      if (text2.length > 0) instructions.push(text2);
      continue;
    }
    if (message.role === "assistant") {
      const text2 = flattenText3(message.content);
    ...
    for (const result of toolResults) {
      input.push({
        ...
        // Empty tool output still needs SOME content on the wire.
        output: flattenText3(result.content) || "(no output)"
      });
    }
    const text = flattenText3(message.content);
...
function mapResponsesUsage(usage) {
  return {
    prompt_tokens: usage.input_tokens ?? 0,
    completion_tokens: usage.output_tokens ?? 0,
    ...usage.input_tokens_details?.cached_tokens !== void 0 && usage.input_tokens_details.cached_tokens > 0 ? { prompt_tokens_details: { cached_tokens: usage.input_tokens_details.cached_tokens } } : {},
    ...usage.output_tokens_details?.reasoning_tokens !== void 0 && usage.output_tokens_details.reasoning_tokens > 0 ? { completion_tokens_details: { reasoning_tokens: usage.output_tokens_details.reasoning_tokens } } : {}
  };
}
function finishReasonOf(response, sawToolCall) {
...
async function* responsesEventsToWire(payloads) {
  const toolItemIds = /* @__PURE__ */ new Map();
  let nextToolIndex = 0;
  let pendingUsage;
  ...
  for await (const payload of payloads) {
    if (payload === DONE) {
      throw new LlmError5("Responses SSE stream ended without response.completed", "STREAM_CLOSED");
    }
    let event;
    try {
      event = JSON.parse(payload);
    } catch {
      throw new LlmError5(`malformed Responses SSE payload: ${payload.slice(0, 120)}`, "MALFORMED_RESPONSE");
    }
    ...
    switch (event.type) {
      ...
      case "response.output_item.added": {
        ...
        if (item?.type === "function_call" && item.id !== void 0 && item.name !== void 0) {
          const wireIndex = nextToolIndex++;
      ...
      case "response.function_call_arguments.delta": {
        if (typeof event.delta === "string") {
          const wireIndex = event.item_id !== void 0 ? toolItemIds.get(event.item_id) : void 0;
          yield JSON.stringify({
            choices: [{
              index: 0,
              delta: {
      ...
      case "response.completed": {
        ...
        if (response?.usage !== void 0) pendingUsage = mapResponsesUsage(response.usage);
      ...
      case "error": {
        throw new LlmError5(event.message ?? "Responses stream error", "UPSTREAM");
      }
      default:
        continue;
    }
...
var CHAT_PROBE_DEFAULT_TIMEOUT_MS = 2e4;
var TOOL_PROBE_DEFAULT_TIMEOUT_MS = 3e4;
...
var PROXY_AGENTS = /* @__PURE__ */ new Map();
...
function gatewayFetch(url, init, proxyUrl) {
  if (proxyUrl === void 0 || proxyUrl.length === 0) return fetch(url, init);
  return undiciFetch(url, {
    ...init,
    dispatcher: proxyAgentFor(proxyUrl)
  });
}
function gatewayHeaders(custom, authorization, contentType, accept) {
  return {
    ...custom ?? {},
...
var NewApiAdapter = class extends LlmAdapter {
  ...
  resolveModel(provider, model, _signal) {
    ...
    const guessed = configured?.reasoningEfforts !== void 0 && configured.reasoningEfforts.length > 0 ? void 0 : defaultReasoningEffortsFor(model);
    const efforts = configured?.reasoningEfforts !== void 0 && configured.reasoningEfforts.length > 0 ? configured.reasoningEfforts : guessed?.efforts;
    return Promise.resolve({
      ...
      ...efforts !== void 0 && efforts.length > 0 ? {
        reasoning: {
          efforts: efforts.map((effort) => ({
            id: ReasoningEffortId(effort),
            name: effort.charAt(0).toUpperCase() + effort.slice(1)
          })),
          ...configured?.defaultReasoningEffort !== void 0 && configured.reasoningEfforts !== void 0 && configured.reasoningEfforts.includes(configured.defaultReasoningEffort) ? { defaultEffort: ReasoningEffortId(configured.defaultReasoningEffort) } : guessed !== void 0 ? { defaultEffort: ReasoningEffortId(guessed.defaultEffort) } : configured?.reasoningEfforts !== void 0 && configured.reasoningEfforts.length > 0 ? { defaultEffort: ReasoningEffortId(highestEffort(configured.reasoningEfforts)) } : {}
        }
      } : {},
   ...
   */
  async fetchModelList(base, apiKey, connection, signal) {
    let response;
    try {
      ...
      throw new LlmError6(`NewAPI model discovery request to ${base} failed`, "TRANSPORT", { cause: error });
    }
    if (!response.ok) {
      let providerError;
      try {
    ...
    try {
      ...
      throw new LlmError6(`NewAPI model discovery from ${base} returned a malformed body`, "MALFORMED_RESPONSE");
    }
  }
  /**
   * Interrogate one gateway endpoint for the models it advertises, serving
  ...
  async discoverModels(request) {
    ...
    const modelKey = /* @__PURE__ */ new Map();
    const seen = /* @__PURE__ */ new Set();
    const merged = [];
  ...
  async probeConnection(request) {
    ...
    let apiKey;
    try {
      apiKey = request.apiKey !== void 0 ? assertUsableApiKey(request.apiKey, PKG, "the probe credential") : await this.config.resolveApiKey(connection);
    } catch (error) {
      return {
        ok: false,
        reachable: false,
    ...
    let response;
    const proxyUrl = request.proxyUrl !== void 0 && request.proxyUrl.length > 0 ? request.proxyUrl : connection.proxyUrl;
    const headers = request.headers !== void 0 && Object.keys(request.headers).length > 0 ? request.headers : connection.headers;
    try {
      response = await gatewayFetch(`${base}/models`, {
        method: "GET",
      ...
      if (request.signal?.aborted) {
        return { ok: false, reachable: false, latencyMs: latencyMs2, error: "probe aborted" };
      }
      ...
      return { ok: false, reachable: false, latencyMs: latencyMs2, error: `connection to ${base} failed: ${cause}` };
    }
    ...
    const result = {
      ...
      latencyMs,
      status: response.status,
      ...proxyUrl === void 0 ? {} : { proxyUsed: proxyUrl }
    };
    if (response.status === 401 || response.status === 403) {
      ...
      try {
        ...
        result.sampleModels = ids.slice(0, 5);
      } catch {
        result.modelCount = 0;
        result.sampleModels = [];
      }
    }
    if (result.authValid === true && request.chatModel !== void 0) {
      result.chat = await this.probeChat(base, apiKey, proxyUrl, headers, request, connection.protocol);
    }
    if (result.authValid === true && request.toolCallModel !== void 0) {
   ...
   */
  async probeChat(base, apiKey, proxyUrl, headers, request, protocol) {
    ...
    try {
      ...
      const chatBody = wire === "anthropic" ? { model: request.chatModel, max_tokens: 5, messages: [{ role: "user", content: "Reply with exactly: ok" }] } : wire === "responses" ? { model: request.chatModel, input: [{ role: "user", content: "Reply with exactly: ok" }], max_output_tokens: 5 } : {
      ...
      const text = wire === "anthropic" ? body.content?.[0]?.text : wire === "responses" ? body.output_text ?? body.content?.[0]?.text : body.choices?.[0]?.message?.content;
      const finishReason = wire === "responses" ? body.status === "incomplete" ? "length" : "stop" : body.choices?.[0]?.finish_reason;
      return {
        ok: true,
        status: response.status,
        latencyMs,
        ...text !== void 0 && text !== null ? { text } : {},
      ...
      if (controller.signal.aborted) {
        return {
          ok: false,
          latencyMs,
          error: `chat probe timed out after ${Math.round(timeoutMs / 1e3)}s`
        };
      }
      const cause = error instanceof Error && error.cause instanceof Error ? error.cause.message : error instanceof Error ? error.message : String(error);
   ...
   */
  async probeToolCall(base, apiKey, proxyUrl, headers, request, protocol) {
    const started = Date.now();
    ...
    try {
      ...
      const toolName = wire === "anthropic" ? body.content?.find((block) => block.type === "tool_use")?.name : wire === "responses" ? body.output?.find((item) => item.type === "function_call")?.name : body.choices?.[0]?.message?.tool_calls?.[0]?.function?.name;
      return toolName !== void 0 && toolName.length > 0 ? { ok: true, status: response.status, latencyMs, toolName } : {
        ok: false,
        status: response.status,
        latencyMs,
      ...
      if (controller.signal.aborted) {
        return {
          ok: false,
          latencyMs,
          error: `tool-call probe timed out after ${Math.round(timeoutMs / 1e3)}s`
        };
      }
      const cause = error instanceof Error && error.cause instanceof Error ? error.cause.message : error instanceof Error ? error.message : String(error);
  ...
  async fetchModelsDevParams(request, signal) {
    ...
    try {
      ...
      if (error instanceof LlmError6) throw error;
      if (signal.aborted) throw error;
    ...
    try {
      ...
      try {
        ...
        if (options.signal?.aborted) {
          throw new LlmError6("NewAPI request aborted by caller", "ABORTED", { cause: error });
        }
        if (error instanceof LlmError6) throw error;
        throw new LlmError6(`NewAPI stream from ${connection.baseURL} failed`, "TRANSPORT", { cause: error });
      } finally {
  ...
  async *request(options, signal, connection, apiKey, onComment) {
    ...
    if (!response.body) {
      throw new LlmError6("NewAPI returned no response body", "EMPTY_RESPONSE");
    }
   ...
   */
  async *anthropicRequest(options, signal, connection, apiKey, onComment) {
   ...
   */
  async *responsesRequest(options, signal, connection, apiKey, onComment) {
   ...
   */
  async raiseForResponse(response, base) {
    ...
    const id = requestId(response.headers);
    throw new LlmError6(message, httpErrorCode(response.status, providerError), {
      status: response.status,
...
function registerChannelConnParser(type, parser) {
  PARSERS.set(type, parser);
}
function parseChannelConn(raw) {
  if (raw === null || typeof raw !== "object" || Array.isArray(raw)) {
    return { ok: false, error: "channel connection descriptor must be a JSON object" };
  }
  const obj = raw;
  const type = typeof obj._type === "string" ? obj._type : void 0;
  if (type === void 0) {
...
var ROUTE_PREFIX = "newapi";
var REF_PREFIX = "newapi";
...
function refForEntry(instance) {
  const named = instance.apiKeyEnv?.trim();
  return named !== void 0 && named.length > 0 ? credentialRef(named) : refOf(instance.id);
}
...
var HEADER_NAME_PATTERN = /^[A-Za-z0-9!#$%&'*+\-.^`|~]+$/;
...
var proxySchema = z.object({
  ...
  enabled: z.boolean()
}).default({ mode: "system", url: DEFAULT_PROXY_URL, enabled: false });
function proxyModeOf(raw) {
  if (raw === void 0) return { mode: "system", url: DEFAULT_PROXY_URL };
  if (raw.mode !== void 0) return { mode: raw.mode, url: raw.url ?? DEFAULT_PROXY_URL };
...
function normalizeSystemProxyValue(raw) {
...
var systemProxyCache;
var SYSTEM_PROXY_TTL_MS = 3e4;
function systemProxyUrl() {
  const now = Date.now();
  if (systemProxyCache !== void 0 && now - systemProxyCache.at < SYSTEM_PROXY_TTL_MS) {
    return systemProxyCache.url;
  }
  const envUrl = process.env.HTTPS_PROXY ?? process.env.https_proxy ?? process.env.HTTP_PROXY ?? process.env.http_proxy;
  let url = envUrl !== void 0 && envUrl.trim().length > 0 ? normalizeProxyUrl(envUrl) : void 0;
  if (url === void 0 && process.platform === "win32") {
    try {
      ...
      if (on !== void 0 && parseInt(on, 16) === 1) {
        ...
        if (raw !== void 0 && raw.length > 0) url = normalizeSystemProxyValue(raw);
      }
    } catch {
    }
  }
...
var Config = z.object({
  ...
  headers: z.dict(z.string()).default({}),
  ui: z.object({
    undoMs: z.number().step(500).min(1e3).max(6e4).default(7e3),
    undoEnabled: z.boolean().default(true),
    soundEnabled: z.boolean().default(true),
    deleteRecoverHint: z.boolean().default(true)
  }).default({ undoMs: 7e3, undoEnabled: true, soundEnabled: true, deleteRecoverHint: true }),
...
function resolveAdapterOptions(config, environment, ref = credentialRef(API_KEY_REF)) {
  ...
  for (const [rawName, rawValue] of Object.entries(config.headers ?? {})) {
    ...
    if (!HEADER_NAME_PATTERN.test(name2)) {
    ...
    if (typeof rawValue !== "string" || !isValidHeaderValue(rawValue)) {
      throw new Error(`${PKG}: custom header "${name2}" has an invalid value (no CR/LF allowed)`);
    }
    headers[name2] = rawValue.trim();
  }
  ...
  return {
    ...
    apiKeyRef: ref,
    ...instanceConfig.protocol === "anthropic" ? { protocol: "anthropic" } : {},
    ...instanceConfig.protocol === "responses" ? { protocol: "responses" } : {},
    ...instanceConfig.keys !== void 0 && instanceConfig.keys.length > 0 ? {
...
function instanceEntriesOf(raw) {
  if (raw.instances !== void 0 && raw.instances.length > 0) {
  ...
  const hasLegacy = raw.baseURL !== void 0 || (raw.models?.length ?? 0) > 0;
  if (!hasLegacy) return [];
  return [{ id: "default", displayName: "NewAPI", instance: { id: "default", ...raw } }];
}
function apply(ctx, config) {
  let current = () => config;
  const entryList = () => instanceEntriesOf(current());
  const states = /* @__PURE__ */ new Map();
  const optionsFor = (id) => {
    const raw = current();
    const entry = entryList().find((candidate) => candidate.id === id);
    if (entry === void 0) throw new Error(`${PKG}: instance "${id}" is no longer configured`);
    const state = states.get(id) ?? {};
    states.set(id, state);
    if (raw === state.lastRaw && state.lastGood !== void 0) return state.lastGood;
    try {
      ...
      if (state.lastGood === void 0) throw error;
      state.lastRaw = raw;
  ...
  const resolveApiKey = async (connection, keyRef) => {
    ...
    throw new LlmError7(
      `${PKG}: no API key for provider route "${ROUTE_PREFIX}"; configure it on the NewAPI settings page in dsh web (credentials reference "${ref}")`,
      "MISSING_CREDENTIAL"
    );
  };
  let indexCache;
  ...
  const syncRegistrations = () => {
    ...
    for (const [index, entry] of entries.entries()) {
      ...
      const flatLegacy = (section.instances === void 0 || section.instances.length === 0) && (section.baseURL !== void 0 || (section.models?.length ?? 0) > 0);
      ...
      const pathChanged = existing.path.length !== path.length || existing.path.some((segment, at) => segment !== path[at]);
  ...
  ctx.llm.registerModelDiscovery(NS, (request) => {
    ...
    return first === void 0 ? Promise.resolve([]) : first.discoverModels(request);
  });
    ...
    cctx.effect(() => connection.rpc.handle(
      ...
      (endpoint, payload, signal) => {
        if (endpoint === "parse-channel-conn") {
          ...
          return Promise.resolve(result.ok ? { ok: true, value: result } : {
            ok: false,
            error: { code: "internal", message: result.error, details: {} }
          });
        }
        if (endpoint === "probe") {
          const request2 = payload;
          const adapter2 = registrations.values().next()?.value?.adapter;
          if (adapter2 === void 0) {
            return Promise.resolve({
              ok: false,
              error: {
          ...
          return adapter2.probeConnection({ ...request2, signal }).then((value) => ({ ok: true, value })).catch((error) => ({
            ok: false,
            error: {
              code: "internal",
...
export {
  ...
  refForEntry,
  refOf,
  registerChannelConnParser,
  resolveAdapterOptions,
  responsesEventsToWire,
  routeOf,
  sanitizeInstanceId,
