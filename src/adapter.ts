  ...
  async probeConnection(request: ProbeRequest): Promise<ProbeResult> {
    ...
    try {
      apiKey = request.apiKey !== undefined
        ? assertUsableApiKey(request.apiKey, PKG, 'the probe credential')
        : await this.config.resolveApiKey(connection)
    } catch (error) {
      // No usable key: the probe cannot authenticate, so it is inconclusive
      // about reachability — report it as a failed probe with the reason.
      return {
        ok: false,
        reachable: false,
    ...
    try {
      ...
      if (request.signal?.aborted) {
        return { ok: false, reachable: false, latencyMs, error: 'probe aborted' }
      }
      const cause = error instanceof Error && error.cause instanceof Error
        ? error.cause.message
        : error instanceof Error ? error.message : String(error)
      return { ok: false, reachable: false, latencyMs, error: `connection to ${base} failed: ${cause}` }
    }
    const latencyMs = Date.now() - started
    ...
    if (response.status === 401 || response.status === 403) {
      ...
      // Snapshot a few advertised ids so a human can sanity-check the gateway
      // without opening it. A malformed body does not downgrade the 200: the
      // endpoint still proved reachability + auth, which is the probe's point.
      try {
        const list = await response.json() as WireModelList
        const ids = (list.data ?? [])
          .map(entry => entry?.id)
